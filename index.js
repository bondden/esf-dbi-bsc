...
class DBIBsc {
  ...
  archiveClass(className) {
    let transactionId = arguments.length <= 1 || arguments[1] === undefined ? '' : arguments[1];
    let pth = arguments.length <= 2 || arguments[2] === undefined ? '' : arguments[2];
    ...
    return new Promise((rs, rj) => {
      ...
      H.db.class.get(className).then(r => {
        ...
        try {
          ...
          //process records
          H.db.select().from(className).all().then(recs => {
